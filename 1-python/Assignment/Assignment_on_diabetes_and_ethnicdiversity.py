# -*- coding: utf-8 -*-
"""
Created on Wed May 31 08:32:49 2023
...
df['Number of times pregnant'][8]

df.describe()

df2 = df2.rename({'Number of times pregnant':'Times pregnant','Plasma glucose concentration':'Glucose concentration in plasma'},axis=1)
df2

df2 = df2.rename({'Number of times pregnant':'Times pregnant','Plasma glucose concentration':'Glucose concentration in plasma'},inplace=True)
df2

df1 = df.drop(df.index[3])

df1 = df.drop(df.index[[2,6]])
...
df1 = df.drop(df.columns[[2]],inplace = True)

list_col = ['Age (years)','Class variable']
df1 = df.drop(list_col,axis=1)
...
#Write a numpy program to test whetehe none of the elements of givem array
x = np.array([1,2,3,4])
print("Original Array: ",x)
print("Test if none of the elements of the said array is zero:- ",np.all(x))

x = np.array([1,0,0,0])
print("Test if none of the elements of the said array is zero:- ",np.any(x))
print((np.all(x)))
...
#write s Numpy program to test a give array elemeyn for finiteness 
import numpy as np 
